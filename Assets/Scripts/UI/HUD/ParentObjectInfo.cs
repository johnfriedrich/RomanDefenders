...
public class ParentObjectInfo : MonoBehaviour {
    ...
    private void Show() {
        ...
        if (selectedObject is Character) {
            Character temp = (Character)selectedObject;
        ...
        if (selectedObject is Entity) {
            Entity temp = (Entity)selectedObject;
            SetStars(temp.CurrentLevel);
            entityInfoDamageLabel.SetActive(true);
            entityInfoArmorLabel.SetActive(true);
            entityInfoDamage.text = temp.CalculateDamage().ToString();
            entityInfoArmor.text = temp.GetArmor().ToString();
            return;
        }
