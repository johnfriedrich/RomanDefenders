...
namespace Entities {
    public class Entity : ParentObject, ISaveLoad {
        ...
        public EntityBehaviour EntityBehaviourClass => entityBehaviourClass;

        public string EntityName => objectName.ToString();

        public int FoodValue => foodValue;

        public SoundHolder FightSound => fightSoundHolder;
        ...
        public override void TakeDamage(float damage) {
            ...
            Sound.Sound.Instance.PlaySoundClipWithSource(takeDamageSoundHolder, audioSource, 0);
        ...
        protected override void OnEnable() {
            ...
            if (IsPlayer() && !(this is Character) && !(entityBehaviourClass is Catapult)) {
                MeshRenderer.materials[2].color = Manager.Manager.Instance.TeamColour;

            }
            if (!IsPlayer()) {
                MeshRenderer.materials[2].color = Manager.Manager.Instance.EnemyColor;
            }
            if (IsPlayer()) {
                entityBehaviourClass.SetTeamColor(Manager.Manager.Instance.TeamColour);
            } else {
                entityBehaviourClass.SetTeamColor(Manager.Manager.Instance.EnemyColor);
            }
            agent.speed = movementSpeed;
        }
        ...
        private void SetDestinationWrapper(Vector3 position) {
            if (agent.enabled) {
                if (!agent.SetDestination(position)) {
                    Debug.Log("Path not found" + objectName);
                }
            }
        }
        ...
        private void PlayDeathAnimation() {
            Color materialColor;
            if (IsPlayer()) {
                materialColor = Manager.Manager.Instance.TeamColour;
            } else {
                materialColor = Manager.Manager.Instance.EnemyColor;
            }
